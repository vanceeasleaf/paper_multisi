...
# @Last Modified time: 2017-10-28 11:59:02
...
with fig('bands_phonopy.eps'):
    ...
    for i in range(2):
        for j in range(3):
            ...
            axes[i, j].plot(
                X - k[fil][::-1], fq[fil][::-1], color='k', alpha=0.9)
            fil = (k > Y - 0.02) * (k < G2 + 0.02)  # Y->G
            axes[i, j].plot(
                G2 - k[fil][::-1] + LX, fq[fil][::-1], color='k', alpha=0.9)
            axes[i, j].text(
                .05, .95, label, transform=axes[i, j].transAxes, **text_style)
            axes[i, j].set_xticks([])
            ...
            for ti, tt in enumerate(t):
                if ti == 0 or ti == len(t) - 1:
                    continue
                axes[i, j].axvline(
                    x=tt, linestyle='--', linewidth=.5, color='black')
    fi.subplots_adjust(
