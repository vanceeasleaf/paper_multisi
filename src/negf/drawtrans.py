...
# @Last Modified time: 2017-06-16 15:37:32
...
with fig('trans.png'):
	...
	for i,v in enumerate(vs):
		ax=axes[i//2,i%2]
		file=v+"/transmission.txt"
		f=np.loadtxt(file,skiprows=1)
		freq=f[:,0]
		trans=f[:,3]
		atoms=io.read(v+"/center/SPOSCAR")
		cross=np.linalg.norm(atoms.cell[1])*(atoms.positions[:,2].max()-atoms.positions[:,2].min())
