...
% @Last Modified time: 2017-06-26 23:08:02
\documentclass[%
 reprint,
 amsmath,amssymb,
 aps,
...
Where $c_{k \sigma}$ means heat capacity of phonon mode $k \sigma$, $v^{\alpha}$ present for mode group velocity and $tau_{k \sigma}$ is phonon lifetime. The anisotropy could derive from these factors and it's worth finding the most important ones.

Heat capacity has no contribution to anisotropy. The average heat capacity is shown in Fig.\ref{fig:cv} and it's obviously that there's no difference in the value of the two directions.

\begin{figure}[b]
  \includegraphics[width=0.48\textwidth]{images/cv}
  \caption{\label{fig:cv} (color online)Average Phonon heat capacity. The red bar present for the value of zigzag direction and yellow represent for that of armchiar direction. }
\end{figure}
